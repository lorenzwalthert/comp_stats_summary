# Variable Selection - Ridge Regression an Lasso
...
In which case $\hat{\beta}_0 = \bar{Y} - \hat{\beta}_1\bar{X} = \bar{Y}$ and can
...
Hence, if there are two variables that are highly correlated, we can expres
...
the norm is smaller. Hence, $x = 1$ corresponds to the OLS solution. On the
