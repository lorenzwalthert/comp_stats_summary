...
## Additive Models ### Structure One assumption we can make is to assume a
particular functional form of $g(\cdot)$, namely an *additive*. That is 
...
### Additive Models in R You can use the package **mgcv** 
```{r} 
...
### Details for Dummies Note that by using reflective pairs $\big\{(x_j - d)_+
...
## Example Let us consider the simple case of a one dimensional predictor space.
...
## Neural Networks Neural networks are high-dimensional non-linear regression
...
### Fitting Neural Networks (in R) The `nnet` function from the package with the
...
yields $0.7(X_1 + X_2) - 0.7(X_1 - X_2)$.
...
The bottom right picture shows the fit with the projection pursuit approach, 
which resemble the original data pretty well. Again, the idea is to use a 
linear model to model interactions properly by first projecting the predictors
optimally.
