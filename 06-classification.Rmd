...
regression, where we have $m(x) = E[Y | X = x]$, we want to assign class abc
...
a natural interpretation of the reponse as a probability. Note that we want
...
Bernulli distribution, ie. $Y_1, ..., Y_n \sim \textrm{Bernulli}(\pi(x_i))$. 
...
* A response distribution (i.e. Bernulli)
...
* The logistic regression assumes the log-odds to be *linear* in the predictors, 
  ie. $\log\Big(\frac{\pi}{1-\pi}\Big) = \sum\limits_{i = 1}^p \beta_i x_i$.
...
## Multiclass case (J > 2)
With logisitc regression, you can not model multiclass cases directly, but 
indirectly using one of the following mehtods:
...
* Similarly, you can also model *all against the reference*, 
  ...
  different group memeberships with a reference group. 
