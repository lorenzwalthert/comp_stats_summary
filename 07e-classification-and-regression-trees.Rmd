...
restrictions to the shape of the partitions and the strategy to choose them, 
...
3. We again refine the current partition as in step 2 by splitting up *one* 
  ...
  $$\mathcal{P} = \mathcal{P}_{old} \setminus \mathcal{P}_{refined} \;\cup\{R_{left}, R_{right}\} $$
...
(\#eq:treeset)
\end{equation}
Just as with Mallow's $C_p$, we can evaluate compute a score for every model
...
is the same or a subeset of the set in equation \@ref(eq:treeset).
...
* Can deal with missing values thanks to the *surrogate split*. For each node
  ...
  observation to be classified, the surrogate is used to *send* the observation
